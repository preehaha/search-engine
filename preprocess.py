...
def preprocess():
    ...
    try:
        df1 = pd.read_excel("./SIRE_dataset/REFERENCE_Checklist_VIQ_Chapter_4_Revision_9_new_approach.xlsx",
                            sheet_name=0, skiprows=[0, 1])
        ...
        # rename columns
        df1.rename(
            columns={"VIQ 7 Q.No": "VIQ_No", "Sub \nQ No.": "Sub_VIQ_NO", "ITEM TO CHECK ": "item_check_Keywords_old",
                     "Red Marked": "essential_Keywords", "Primary ": "item_check_Keywords",
                     "Secondary": "nature_check_Keywords", "Support": "new_nature_check_keywords"}, inplace=True)
        pass
    except OSError as e:
        print(e.errno)
        pass
    ...
    # rename columns
    df2.rename(columns={"Viq Ref": "VIQ_No", "Unnamed: 1": "Sub_VIQ_NO", "Inspector Observation": "Observation"},
               inplace=True)
    ...
    dict_replace_item_check = {"off course alarm": "course alarm", "sat c": "sat", "company sm": "company",
                               "ex rated mobile phone": "mobile phone",
                               "electronic chart display and information system": "electronic chart display information system",
                               "position fixing method": "position fixing"}
    ...
    nature_check_remove_list = ['ai', 'not', '2', 'reord', 'note', 'order', 'nm', 'at', 'use', 'ic', 'conn', 'con',
                                'ec', 'on board', 'time', 'low', 'back up', 'dr', 'vt', 'id', 'ME', 'CF', '406', 'cpp',
                                'cf', 'two', 'name', 'off', 'date', 's', 'pi', 'no go', 'ink', 'np232', 'p', 'tss',
                                'as', 'sm', 'pl', 'two way', 'key', 'Off', '3 cm', 'size', 'each', 'on', '16', 'x',
                                'form c', 'way', 'upto', 'new', 'near miss', 'pm', 'pp', 'check', '3', 'number', 'ra',
                                '1w', 'up side', 'available', 's63', 'np 231', 'dpa', 'me', 'back', 'in force', 'good',
                                'oow', 'cm']
    dict_replace_nature_check = {"nav warning": "warning", "sat c": "sat", "notice to mariner": "notice mariner",
                                 "at sea maintenance": "maintenance"}  # method to remove irrelevant keywords
    ...
    df3["essential_Keywords"] = df3.apply(
        lambda x: remove_irrelevant_keywords(x['essential_Keywords'], item_check_remove_list), axis=1)
    df3["item_check_Keywords"] = df3.apply(
        lambda x: remove_irrelevant_keywords(x['item_check_Keywords'], item_check_remove_list), axis=1)
    df3["nature_check_Keywords"] = df3.apply(
        lambda x: remove_irrelevant_keywords(x['nature_check_Keywords'], nature_check_remove_list), axis=1)
    df3["item_check_Keywords"] = df3.apply(
        lambda x: replace_method_keywords(x['item_check_Keywords'], dict_replace_item_check), axis=1)
    df3["nature_check_Keywords"] = df3.apply(
        lambda x: replace_method_keywords(x['nature_check_Keywords'], dict_replace_nature_check), axis=1)
    ...
    df4['filtered_keywords'] = df4.apply(
        lambda x: filter_list_string_essential_keywords(x['parsed_keywords'], x['combined_essential_Keywords']), axis=1)
    ...
    for i in range(len(df4['filtered_keywords'])):
        if df4['filtered_keywords'][i] == []:
            df4['filtered_keywords'] = df4.apply(
                lambda x: filter_list_string(x['parsed_keywords'], x['combined_keywords']), axis=1)
        else:
            continue
    ...
    for i in range(len(df4["filtered_keywords"])):
        if df4["filtered_keywords"][i] == []:
            df4["filtered_keywords"][i] = \
            (find_new_filtered_list_reverse_order(df4["parsed_keywords"][i], df4["combined_keywords"][i]))[0]
        else:
            continue
    ...
    for i in range(len(df4["index_list"])):
        for val in df4["index_list"][i]:
            df_PVSN.loc[k] = [df4["parsed_keywords"][i], df3["VIQ_No"][val], df3["Sub_VIQ_NO"][val],
                              df3["nature_check_Keywords"][val]]
            k += 1
    ...
    for i in range(len(df_PVSN["nature_check_Keywords"])):
        temp1 = list()
        temp1 = find_new_filtered_list_nature_check(df_PVSN["parsed_keywords"].iloc[i],
                                                    df_PVSN["nature_check_Keywords"].iloc[i])
